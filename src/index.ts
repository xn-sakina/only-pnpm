...
export const run = async () => {
  ...
  /**
   * @deprecated `only-pnpm` dependence `which-pm-runs` too old
   * See https://github.com/zkochan/which-pm-runs
   */
  // 1. set only-allow
...
async function getNodeMajorVersion() {
  const version = (await execa(`node`, ['--version'])).stdout.trim().slice(1)
